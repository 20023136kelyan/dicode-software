...
const VideoCard = ({
    ...
    if (isCompletedStatus(item.status)) {
      return (
        <>
          ...
          <Button
            variant="outline"
            onClick={() => onAddShot(item)}
            size="icon"
            aria-label="Add shot"
            className="h-8 w-8 rounded-full border-border text-foreground hover:bg-accent"
          >
  ...
  const shotCount = item.shot_sequence?.shots?.length ?? 0;
  const metaChips = [
    item.seconds ? `${item.seconds}s` : null,
    item.model || null,
    item.size || null,
    shotCount > 0 ? `${shotCount} shot${shotCount > 1 ? "s" : ""}` : null,
          ...
          <div className="flex flex-col gap-3 p-4">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0 space-y-2">
                <div className="mt-1 mr-4 line-clamp-2 break-words text-sm font-semibold leading-5 text-foreground">
